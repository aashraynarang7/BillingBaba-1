...
export default function ConsumptionReportPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Flatten items array from each job work record
        const rows: ConsumptionRow[] = [];
        data.forEach((jobWork) => {
          const parentDate = jobWork.invoiceDate ? new Date(jobWork.invoiceDate) : new Date();
          const items = Array.isArray(jobWork.items) ? jobWork.items : [];
  ...
  const filtered = useMemo(() => {
    ...
    return allRows.filter((row) => row.itemName.toLowerCase().includes(lower));
  }, [allRows, searchTerm]);

  const totals = useMemo(() => {
    const totalQty = filtered.reduce((sum, row) => sum + row.quantity, 0);
  ...
  const handleExcel = async () => {
    ...
    const rows = filtered.map((row, i) => ({
      ...
      "Qty Consumed": `${row.quantity} ${row.unit}`.trim(),
      Amount: row.amount,
    }));
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b">
        <h1 className="text-lg font-semibold text-gray-800">Consumption Report</h1>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-gray-50 border-b">
                ...
                <th className="text-right px-4 py-3 font-medium text-gray-600">QTY CONSUMED</th>
      ...
      <div className="px-6 py-3 border-t bg-gray-50 flex items-center justify-between text-sm font-medium text-gray-700">
        <span>
          Total Qty Consumed: <span className="text-gray-900">{totals.totalQty}</span>
        </span>
        <span>
          Total Amount:{" "}
