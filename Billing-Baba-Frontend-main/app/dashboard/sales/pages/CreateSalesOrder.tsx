...
export default function CreateSaleOrderPage({ onCancel, initialData }: { onCancel: () => void, initialData?: any }) {
    const [orderDate, setOrderDate] = useState<Date | undefined>(initialData?.orderDate ? new Date(initialData.orderDate) : new Date());
    ...
    const [advanceAmount, setAdvanceAmount] = useState(initialData?.receivedAmount || 0);
    ...
    const handleSave = async () => {
        ...
        formData.append('documentType', 'SO');
        ...
        formData.append('receivedAmount', advanceAmount.toString());
        formData.append('balanceDue', (totalAmount - advanceAmount).toString());
                            ...
                            <PopoverContent className="w-[300px] p-0">
                                <Command shouldFilter={false}>
                                    <CommandInput
                                        ...
                                        onValueChange={(val) => {
                                            setPartySearch(val);
                                            // Debounce logic could be added here, currently relies on useEffect
                                        }}
                                    />
                                    <CommandList>
                                        ...
                                        <CommandGroup>
                                            {parties.map((party) => (
                                                <CommandItem
                                                    key={party._id}
                                                    value={party.name} // We value by name for accessibility, but handle by ID
                                                    onSelect={() => {
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            ...
                            <Input
                                ...
                                className="w-48 font-bold text-lg h-11"
                                disabled
                            />
                        </div>
                        <div className="flex items-center justify-end gap-4">
                        ...
                        <div className="flex items-center justify-end gap-4">
                            ...
                            <Input
                                value={(totalAmount - advanceAmount).toFixed(2)}
