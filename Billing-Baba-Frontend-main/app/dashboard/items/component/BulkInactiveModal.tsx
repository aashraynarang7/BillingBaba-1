...
import { fetchBulkItemData, bulkMarkInactive } from '@/lib/api';
...
const BulkInactiveModal = ({ isOpen, onClose, onSuccess }: BulkInactiveModalProps) => {
    ...
    const handleMarkInactive = async () => {
        ...
        setIsSaving(true);
        try {
            await bulkMarkInactive(Array.from(selected));
            toast({ title: `${selected.size} item(s) marked as inactive`, className: 'bg-green-500 text-white' });
            onSuccess?.();
            onClose();
        } catch {
            toast({ title: 'Failed to mark items inactive', variant: 'destructive' });
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[85vh]">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b">
                    <h2 className="text-xl font-bold text-gray-800">Bulk Inactive</h2>
                        ...
                        <table className="w-full text-sm">
                            <thead className="bg-gray-50 sticky top-0">
                                <tr>
                                    ...
                                    <th className="px-4 py-3 text-right font-semibold text-gray-600">Qty Sold (Last 90 Days)</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                                {filtered.length === 0 ? (
                                    <tr>
                                        <td colSpan={4} className="text-center py-10 text-gray-400">No active items found</td>
                                    </tr>
                                ) : (
                                    filtered.map(item => (
                                        <tr key={item._id} className="hover:bg-gray-50 cursor-pointer" onClick={() => toggle(item._id)}>
                                            ...
                                            <td className="px-4 py-3 text-right text-gray-600">{item.qtySold}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
                ...
                <div className="flex items-center justify-between px-6 py-3 border-t bg-gray-50 rounded-b-xl">
                    ...
                    <div className="flex gap-3">
                        ...
                        >
                            ...
                            Mark as Inactive {selected.size > 0 ? `(${selected.size})` : ''}
                        </Button>
                    </div>
                </div>
            </div>
...
export default BulkInactiveModal;
