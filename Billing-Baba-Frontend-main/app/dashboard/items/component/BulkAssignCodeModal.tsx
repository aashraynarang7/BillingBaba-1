...
import { fetchBulkItemData, bulkAssignCode } from '@/lib/api';
...
interface BulkAssignCodeModalProps {
    isOpen: boolean;
    onClose: () => void;
    onSuccess?: () => void;
}

const BulkAssignCodeModal = ({ isOpen, onClose, onSuccess }: BulkAssignCodeModalProps) => {
    ...
    useEffect(() => {
        ...
        fetchBulkItemData()
            .then(data => {
                // Only show items without an item code
                const noCode = data.filter((i: BulkItem) => !i.itemCode);
                setItems(noCode);
            })
    ...
    const handleAssignCode = async () => {
        ...
        setIsSaving(true);
        try {
            await bulkAssignCode(Array.from(selected));
            toast({ title: `Codes assigned to ${selected.size} item(s)`, className: 'bg-green-500 text-white' });
            onSuccess?.();
            onClose();
        } catch {
            toast({ title: 'Failed to assign item codes', variant: 'destructive' });
        } finally {
            setIsSaving(false);
        }
    };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl mx-4 flex flex-col max-h-[85vh]">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b">
                    <h2 className="text-xl font-bold text-gray-800">Assign Item Code</h2>
                    ...
                    {isLoading ? (
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filtered.length === 0 ? (
                                    <tr>
                                        <td colSpan={3} className="text-center py-10 text-gray-400">All items already have item codes</td>
                                    </tr>
                                ) : (
                                    filtered.map(item => (
                ...
                <div className="flex items-center justify-between px-6 py-3 border-t bg-gray-50 rounded-b-xl">
                    <span className="text-xs text-gray-500 bg-white border border-gray-200 rounded px-2 py-1">
                        Showing items that don't have item code
                    </span>
                    <div className="flex gap-3">
                        ...
                        >
                            ...
                            Assign Code {selected.size > 0 ? `(${selected.size})` : ''}
                        </Button>
                    </div>
                </div>
            </div>
...
export default BulkAssignCodeModal;
